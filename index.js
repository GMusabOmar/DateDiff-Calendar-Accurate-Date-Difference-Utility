...
function calculateDateDiff() {
  ...
  if (start > end) {
    if (autoSwapInput.checked) {
      [start, end] = [end, start];
      showMessage("neutral", "📌 Dates were swapped (start was after end).");
    } else {
      showMessage(
        "bad",
  ...
  rangeLine.textContent = `Range: ${formatDate(start)} → ${formatDate(end)}`;
...
btnDemo.addEventListener("click", () => {
  ...
  setStatus("Ready…");
  showMessage("neutral", "📌 Demo dates set (last 90 days). Click Calculate.");
});
